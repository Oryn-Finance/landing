...
import { avalanche, arbitrumSepolia } from 'viem/chains';
import { Result, err, ok } from 'neverthrow';

export type EvmChain = 'avalanche_testnet' | 'arbitrum_sepolia';

export const chainIdToEvmChain: Record<number, EvmChain> = {
...
export const isOrderForSupportedChains = (order: any): boolean => {
  // Extract chain from asset (format: "chain:asset_id")
  const sourceAsset = order.source_intent?.asset || '';
  const destAsset = order.destination_intent?.asset || '';
  
  const sourceChain = sourceAsset.split(':')[0];
  const destinationChain = destAsset.split(':')[0];
...
): Promise<Result<{ message: string; walletClient: WalletClient }, string>> => {
